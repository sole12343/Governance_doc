 ...
 */
/* {Governor} 的扩展，将执行过程绑定到 Compound Timelock。
    这为所有成功的提案（除了投票持续时间之外）增加了一个由外部 timelock 强制执行的延迟。
    对于任何要执行的操作，{Governor} 需要成为 timelock 的管理员。一个公共的，不受限制的，
    {GovernorTimelockCompound-__acceptAdmin} 可用于接受 timelock 的所有权。
    使用这种模型意味着提案将由 {TimelockController} 而不是 {Governor} 进行操作。
    因此，资产和权限必须附加到 {TimelockController}。发送到 {Governor} 的任何资产都将无法访问。*/
